...
// 计算图片实际大小
+ (CGSize)neededSizeForPhoto:(UIImage *)photo sizeString:(NSString *)sizeString {
    // 这里需要缩放后的size
    CGFloat maxFloat = 100;
    if (sizeString != nil) {
        CGSize originalSize = CGSizeFromString(sizeString);
        CGFloat ratio = originalSize.width / originalSize.height;
        if (originalSize.width > originalSize.height) {
            return CGSizeMake(maxFloat * ratio, maxFloat);
        } else {
            return CGSizeMake(maxFloat, maxFloat / ratio);
        }
    }
    if (photo == nil) {
...
+ (CGSize)getBubbleFrameWithMessage:(id <XHMessageModel>)message {
    ...
    switch (message.messageMediaType) {
        ...
        case XHBubbleMessageMediaTypeVideo: {
            CGSize needVideoConverPhotoSize = [XHMessageBubbleView neededSizeForPhoto:message.videoConverPhoto sizeString:message.sizeString];
            bubbleSize = CGSizeMake(needVideoConverPhotoSize.width, needVideoConverPhotoSize.height + kXHNoneBubblePhotoMargin * 2);
            break;
        }
        case XHBubbleMessageMediaTypePhoto: {
            CGSize needPhotoSize = [XHMessageBubbleView neededSizeForPhoto:message.photo sizeString:message.sizeString];
...
- (void)configureMessageDisplayMediaWithMessage:(id <XHMessageModel>)message {
    switch (message.messageMediaType) {
        case XHBubbleMessageMediaTypeText:
            ...
            break;
        case XHBubbleMessageMediaTypePhoto:
            [_bubblePhotoImageView configureMessagePhoto:message.photo sizeString:message.sizeString thumbnailUrl:message.thumbnailUrl originPhotoUrl:message.originPhotoUrl onBubbleMessageType:self.message.bubbleMessageType];
            break;
        case XHBubbleMessageMediaTypeVideo:
            [_bubblePhotoImageView configureMessagePhoto:message.photo sizeString:message.sizeString thumbnailUrl:message.thumbnailUrl originPhotoUrl:message.originPhotoUrl onBubbleMessageType:self.message.bubbleMessageType];
            break;
...
- (void)layoutSubviews {
    ...
    switch (currentType) {
        ...
        case XHBubbleMessageMediaTypeVideo:
        case XHBubbleMessageMediaTypeLocalPosition: {
            CGSize needPhotoSize = [XHMessageBubbleView neededSizeForPhoto:self.message.photo sizeString:self.message.sizeString];
            CGFloat paddingX = 0.0f;
            if (self.message.bubbleMessageType == XHBubbleMessageTypeSending) {
